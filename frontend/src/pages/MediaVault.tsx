...
import { Upload, X, ZoomIn, Heart, Share2, Loader2, ImagePlus } from 'lucide-react';
          ...
          <AnimatePresence mode="popLayout">
            {photos.map((photo, index) => (
              <motion.div
                ...
                className="break-inside-avoid glass rounded-2xl overflow-hidden group relative cursor-pointer"
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                   <div className="flex justify-between items-center mb-2">
                     <h3 className="text-white font-medium text-sm line-clamp-1">{photo.caption}</h3>
                     <div className="flex gap-2 text-gray-400">
                        <Heart size={16} /> <span className="text-xs">{photo.likes}</span>
                     </div>
                   </div>
                   <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest">
                      View Full <ZoomIn size={12} />
                   </div>
                </div>
              </motion.div>
            ))}
